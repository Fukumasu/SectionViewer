...
class Points_GUI(Color_GUI):
    def __init__(self, main):
        ...
        self.entry_nm = ttk.Entry(
            frame, textvariable=self.vars['nm'], width=30)
        ...
        self.entry_r = ttk.Entry(
            rgb_frame, textvariable=self.vars['rs'], width=5)
        self.entry_r.grid(column=2, row=1, padx=3)
        self.entry_g = ttk.Entry(
            rgb_frame, textvariable=self.vars['gs'], width=5)
        self.entry_g.grid(column=2, row=2, padx=3)
        self.entry_b = ttk.Entry(
            rgb_frame, textvariable=self.vars['bs'], width=5)
        self.entry_b.grid(column=2, row=3, padx=3)
        preset_canvas = tk.Canvas(
            rgb_frame, width=160, height=20, cursor='hand2')
        preset_canvas.create_image(0, 0, anchor='nw', image=self.preset_image)
        preset_canvas.grid(column=0, row=4, columnspan=2,
                           padx=20, sticky=tk.SW)
        preset_canvas.bind('<Button-1>', self.preset)
        auto_button = ttk.Button(
            rgb_frame, text='Auto', command=self.auto_color)
        ...
        self.entry_h = ttk.Entry(
            hsl_frame, textvariable=self.vars['hs'], width=5)
        self.entry_h.grid(column=2, row=1, padx=3)
        self.entry_s = ttk.Entry(
            hsl_frame, textvariable=self.vars['ss'], width=5)
        self.entry_s.grid(column=2, row=2, padx=3)
        self.entry_l = ttk.Entry(
            hsl_frame, textvariable=self.vars['ls'], width=5)
        self.entry_l.grid(column=2, row=3, padx=3)
        preset_canvas = tk.Canvas(
            hsl_frame, width=160, height=20, cursor='hand2')
        self.preset_id = preset_canvas.create_image(
            0, 0, anchor='nw', image=self.preset_image)
        preset_canvas.grid(column=0, row=4, columnspan=2,
                           padx=20, sticky=tk.SW)
        preset_canvas.bind('<Button-1>', self.preset)
        auto_button = ttk.Button(
            hsl_frame, text='Auto', command=self.auto_color)
        auto_button.grid(column=1, row=4, columnspan=2, pady=2, sticky=tk.E)
        ...
        self.button_cg = ttk.Button(
            bottom_frame, text='Change', command=self.change)
    ...
    def refresh_tree(self):
        ...
        for i in sort:
            ...
            self.treeview.insert('', 'end', str(
                i), text=' '+names[i], image=self.icons[i])
        self.treeview.selection_set()
    ...
    def set_vars(self, i):
        ...
        text = '(x,y,z) = ({0:.1f}, {1:.1f}, {2:.1f})'.format(
            crs[2], crs[1], crs[0])
    ...
    def change(self):
        ...
        name = obj[i][0]
        pt_show = list(main.display['shown_points'])
        pt_show[i] = False
        main.display['shown_points'] = tuple(pt_show)
        ...
        def repair():
            ...
            main.skeleton_canvas.bind('<Button-1>', main.click_skeleton)
            main.skeleton_canvas.bind('<Motion>', main.track_skeleton)
            ...
            if time.time() - main.click_time < 0.5:
                if main.mode == 1:
                    ...
                    if np.linalg.norm(v) > 0.08:
                        k = int(np.angle(v[0] + 1j * v[1]) / np.pi * 6)
                        k = ['L', 0, 'N', 'N', 0, 'H',
                               'H', 0, 'I', 'I', 0][k]
                        if k != 0:
                            main.position_key('Ctrl+' + k)
                elif main.mode == 2:
        ...
        ttk.Label(main.bottom, text='µm').pack(
            side=tk.RIGHT, anchor=tk.N, padx=5, pady=5)
        main.sbar_entry.pack(side=tk.RIGHT, anchor=tk.N, pady=5)
        ttk.Label(main.bottom, text='Scale bar:').pack(
            side=tk.RIGHT, anchor=tk.N, padx=5, pady=5)
        ...
        main.skeleton_canvas.unbind('<Motion>')
        main.skeleton_canvas.unbind('<Button-1>')

...
class Memories:
    def __init__(self, Hub):
        ...
        self.name_var.trace("w", self.mm_name)
        ...
        button1 = ttk.Button(frame2, text="Add Current", command=self.add_mm)
        button1.pack(side=tk.LEFT, padx=2)
        button2 = ttk.Button(frame2, text="Delete", command=self.del_mm)
        ...
        self.treeview.heading("#0", text="States", anchor=tk.W)
        ...
        self.treeview.bind("<<TreeviewSelect>>", self.mm_select)
        for i in np.argsort(self.names()):
            self.treeview.insert("", "end", str(i), text=" "+self.memories[i]["name"])
        ...
        button3 = ttk.Button(frame5, text="Override", command=self.override_mm)
        button3.pack(pady=2)
        self.button_ov = button3
        button4 = ttk.Button(frame5, text="Restore", command=self.restore_mm)
        button4.pack(pady=2)
    ...
    def __getattr__(self, name):
        if name == "val":
            return object.__getattribute__(self, "memories")
        else:
    ...
    def names(self):
        return [mm["name"] for mm in self.memories]
    
    
    def settings(self):
        gui = self.Hub.gui
        gui.palette.title("Memories")
    ...
    def mm_select(self, event):
        ...
        secv = self.memories[int(self.treeview.selection()[0])]
    ...
    def set_name(self, x, new):
        ...
        self.memories[int(x[0])]["name"] = new
    ...
    def add_mm(self):
        ...
        if idx != -1:
            hist[idx:] = hist[idx:idx+1]
        hist += [[self, [self.del_memories, [x], self.add_memories, [x, [secv]]]]]
        if Hub.hidx_saved > idx:
            Hub.hidx_saved = -1 - len(hist)
        else: 
            Hub.hidx_saved -= idx + 2
        ...
        self.add_memories(x, [secv])
        
        
    def add_memories(self, x, ms):
        for m, i in zip(ms,x):
            self.memories = self.memories[:i] + [m] + self.memories[i:]
    ...
    def del_mm(self):
        ...
        ms = [self.memories[i] for i in x]
        
        hist, idx = Hub.history, Hub.hidx
        
        if idx != -1:
            hist[idx:] = hist[idx:idx+1]
        hist += [[self, [self.add_memories, [x, ms], self.del_memories, [x]]]]
        if Hub.hidx_saved > idx:
            Hub.hidx_saved = -1 - len(hist)
        else: 
            Hub.hidx_saved -= idx + 2
    ...
    def del_memories(self, x):
        for i in x[::-1]:
            self.memories = self.memories[:i] + self.memories[i+1:]
    ...
    def override_mm(self):
        ...
        if idx != -1:
            hist[idx:] = hist[idx:idx+1]
        hist += [[self, [self.change_memory, [x, old], self.change_memory, [x, new]]]]
        if Hub.hidx_saved > idx:
            Hub.hidx_saved = -1 - len(hist)
        else: 
            Hub.hidx_saved -= idx + 2
        ...
        self.change_memory(x, new)
        
        
    def change_memory(self, x, secvs):
        for i, secv in zip(x, secvs):
            self.memories[i] = secv
        self.pre_selection = []
    ...
    def restore_mm(self):
        ...
        select = self.treeview.selection()
        if len(select) == 1:
            secv = self.memories[int(select[0])]
            try:
                dat = secv["data"]
            ...
            for s in select:
                pts += [[p[0], [p[1][0], p[1][1], p[1][2]], 
                       [p[2][0], p[2][1], p[2][2]]] for p in self.memories[int(s)]["points"]]
